...
| [`review-loop`](skills/review-loop/SKILL.md) | Iteratively review and improve a plan with independent reviewer subagents. Uses deterministic per-pass review artifacts (`REVIEW-P1.md`, `REVIEW-P2.md`, ...), finding IDs, bounded passes (`max_passes=3` by default), and an explicit disposition trail that can close cleanly, close non-actionable findings with evidence, or report an incomplete loop. |
